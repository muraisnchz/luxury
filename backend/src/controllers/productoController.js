...
const crearProducto = async (req, res) => {
  try {
    ...
    console.error("Error al crear producto. Detalles:", error);
    res
      .status(500)
...
const modificarProducto = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ mensaje: "Error al modificar", error: error.message });
  }
};
...
const bajaLogicaProducto = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ mensaje: "Producto retirado del catálogo", producto });
  } catch (error) {
    res
      .status(500)
      .json({ mensaje: "Error al dar de baja", error: error.message });
  }
};
